...
############################################################################
        ...
        user8= df[df['user']=='U8']
        user9= df[df['user']=='U9']
        user10= df[df['user']=='U10']
        user11= df[df['user']=='U11']
        user12= df[df['user']=='U12']
        user13= df[df['user']=='U13']
        df_train= pd.concat([user1,user2, user3, user4, user7])
        
        set_config(display='diagram')
        model
        df_train.drop('user', axis=1, inplace= True)
...
##########################################################################
    ...
    * or just standing still
    ...
    ''')
        ...
        weight = st.slider("Your weight(in kilograms)", 50, 120)
        bmi = weight / (height * height)

        velocity = st.slider("How fast did you go? (Meters per second))", 1,10)
